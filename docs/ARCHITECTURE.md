...
- The rolling budget currently counts gateway refreshes, not each HTTP retry. Lock lifetime is fixed while upstream timeout and retries are configurable. Verify these limits before relying on the configured number as a strict provider quota.
